...
class BASFData(NamedTuple):
    """Container for BASF stock and options data."""
    stock_df: pd.DataFrame
    options_df: pd.DataFrame
    stock_table: 'FinancialTable'
    options_table: 'FinancialTable'


def read_basf_options(filepath: str = 'Datasets/Options_BASF/basf_optionsdaten.xlsx', 
                     style: str = 'assumptions') -> BASFData:
    """
    Convenience function to read BASF options Excel file.
    
    Returns separate DataFrames and styled tables for stock and options data.
    
    Args:
        filepath: Path to BASF Excel file
        style: Style preset to apply
    
    Returns:
        BASFData named tuple with:
            - stock_df: DataFrame with BASF Aktie data
            - options_df: DataFrame with Optionsdaten
            - stock_table: Styled FinancialTable for stock data
            - options_table: Styled FinancialTable for options data
    ...
    tables = reader.read_all_tables('BASF Optionsdaten', style=style)
    
    # Expecting exactly 2 tables: BASF Aktie and Optionsdaten
    if len(tables) != 2:
    ...
    if 'Aktie' not in stock_name:
        raise ValueError(f"First table should be 'BASF Aktie', got '{stock_name}'")
    if 'Option' not in options_name:
        raise ValueError(f"Second table should be 'Optionsdaten', got '{options_name}'")
...
def read_basf_returns(filepath: str = 'Datasets/Modelle_S2_S3/Modul_4_5.Marius.Ciepluch.xlsx',
                      sheet: str = None,
                      auto_detect_header: bool = True) -> pd.DataFrame:
    """
    Read BASF returns data from Excel with automatic header detection.

    Args:
        filepath: Path to Excel file
        sheet: Sheet name containing returns data (if None, shows available options)
        auto_detect_header: If True, automatically detect header row (default: True)

    Returns:
        DataFrame with returns data

    Example:
        >>> from finmodel import read_basf_returns, print_tables
        >>> # List available tables first
        >>> print_tables('Datasets/Modelle_S2_S3/Modul_4_5.Marius.Ciepluch.xlsx')
        >>> # Then read specific sheet
        >>> returns = read_basf_returns(sheet='Rendite BAS')
        >>> returns.head()
    """
    if sheet is None:
        # Show available tables to help user
        print_tables(filepath, glimpse=True, max_rows=2)
        raise ValueError("Please specify sheet= parameter from the tables listed above")

    if auto_detect_header:
        # Use openpyxl to detect header row
        wb = openpyxl.load_workbook(filepath, read_only=True, data_only=True)
        ws = wb[sheet]
        ...
        for row_idx in range(1, max_row + 1):
            ...
            string_count = sum(1 for v in row_values if isinstance(v, str) and v is not None)

            if string_count >= len([v for v in row_values if v is not None]) * 0.5:
                # Check next row to see if it has data (numbers)
                if row_idx < ws.max_row:
                    next_row = [ws.cell(row_idx + 1, col).value for col in range(1, ws.max_column + 1)]
                    numeric_count = sum(1 for v in next_row if isinstance(v, (int, float)) and v is not None)

                    if numeric_count > 0:
                        header_row = row_idx - 1  # pandas uses 0-indexed header
                        break
        ...
        # Read normally (assumes first row is header)
